...
class UserService {
  ...
  *auth(login, password) {
  ...
  get(login) {
    ...
    return this._request('get', url).then(users => users[0]).then(formatUser);
  }
  ...
  list(logins) {
    const that = this;
    const usersPromises = logins.map(name => that.get(name));
    return Promise.all(usersPromises).then(users => users.filter(user => !!user)).catch(() => {
      console.log(22222);
    });
  }
